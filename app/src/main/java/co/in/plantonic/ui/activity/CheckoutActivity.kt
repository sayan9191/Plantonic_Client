...
import co.`in`.plantonic.databinding.ActivityCheckoutBinding
import co.`in`.plantonic.firebaseClasses.AddressItem
import co.`in`.plantonic.ui.dialogbox.LoadingScreen.Companion.hideLoadingDialog
import co.`in`.plantonic.ui.dialogbox.LoadingScreen.Companion.showLoadingDialog
        ...
        viewModel.currentAddress.observe(this) { currentAddress ->
            if (currentAddress != null && currentAddress === addressItem) {
                ...
                if (addressItem.landmark != ""){
                    customerAddress += ", Near - ${addressItem.landmark}"
                }
                intent.putExtra(
